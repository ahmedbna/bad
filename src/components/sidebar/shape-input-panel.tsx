...
export const ShapeInputPanel = ({
  ...
  // Update the processPropertyInput function to handle all shape types and update coordinates
    ...
    switch (selectedTool) {
      case 'line':
        if (propertyInput.length && step === 1) {
          ...
          const directionRad = directionDeg * (Math.PI / 180); // degrees to radians
      ...
      case 'polygon':
        if (propertyInput.radius && propertyInput.sides && step === 1) {
          ...
          if (!isNaN(radius) && !isNaN(sides)) {
            ...
            completeShape([basePoint], { radius, sides });
            setStep(0);
          }
        }
        break;
      ...
      case 'arc':
        if (
          ...
          if (!isNaN(radius) && !isNaN(startAngle) && !isNaN(endAngle)) {
            ...
            completeShape([basePoint], { radius, startAngle, endAngle });
            setStep(0);
          }
        }
        break;
      ...
      case 'spline':
        if (step >= 2 && propertyInput.tension) {
          ...
          if (!isNaN(tension)) {
            // For spline, just update the property without completing
            setTempShape({
              id: 'temp-shape',
              type: 'spline',
  ...
  // Fix handleInputChange to always update both coordinate and property inputs
    ...
    if (field === 'x' || field === 'y') {
      ...
      if (field === 'radius' && e.target.value) {
        ...
        if (!isNaN(radiusValue)) {
          setPropertyInput((prev) => ({
            ...prev,
            diameter: (radiusValue * 2).toFixed(2),
          }));
        }
        ...
        if (!isNaN(diameterValue)) {
          setPropertyInput((prev) => ({
            ...prev,
            radius: (diameterValue / 2).toFixed(2),
          }));
        }
      }

      // Immediately try to update the temp shape preview
      if (drawingPoints.length > 0) {
        // Small delay to allow the state to update
        setTimeout(() => updateTempShapeFromProperties(), 0);
      }
    }
  };

  // Enhanced function to update property inputs from coordinates
